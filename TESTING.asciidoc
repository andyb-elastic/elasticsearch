...
Specify the image IDs of the Windows boxes to gradle with the properties:
...
If running vagrant directly, specify the Windows image IDs in the environment variables:
...
may not mount the packaging test project in the right place. To see the up tasks for
all the boxes you have configured, run `./gradlew :qa:vagrant:tasks --all`. Once the
VM is up, ssh into it
...
To run the portable packaging tests, again inside the VM
